...
const ReservaInstalacionCupos = ({cupos,loading,getReservaDetail,selecReservaCupo,selectedCupos}:{
    ...
    const [hoursCupo,setHoursCupo] = useState(hours)
    ...
    const mergeData = () =>{
        // console.log(cupoTime)
        // console.log(hoursCupo)
        // console.log(cupos)
        // console.log(now.year()()${now.month()}ow.month(),now.day())

        // const reservasCupo = newHoursCupo.map(item=>item.reserva)
        const d = groupByToMap<CupoReserva,number | null>(cupos,(item)=>item.reserva_id)
            let prevColors:string[] = []
             d.forEach((items,key)=>{
                if(key != null){
    ...
    useEffect(()=>{
        ...
        if(newCupos.length >0){
            const newHoursCupo =  hoursCupo.map(item=>{
        ...
        <div className="mt-2">
            {hoursCupo.map((item,idx)=>{
               const isSelected = selectedCupos.map(cupo=>moment(cupo.time).utc().format()).includes(moment.utc(item.hour).format())
                const isAfter =moment.utc(item.hour).isBefore(moment().utcOffset(0, true).toJSON(),'hours')
                return(
                            ...
                            `
                               ...
                               <>
                                ...
                                {item.reserva == undefined ?
                                    <span className="text-red-700 text-xs">Hora no configurada</span>
                                    :
                                    <>
