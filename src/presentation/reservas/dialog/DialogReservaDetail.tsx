...
import { DeleteReserva } from "@/core/repository/reservas";

const DialogReservaDetail = ({
  open,
  close,
  data,
  update,
  ...
  const [openDeleteReservaConfimation, setOpenDeleteReservaConfirmation] =
    useState(false);

  const deleteReserva = async () => {
    try {
      setOpenDeleteReservaConfirmation(false);
      close();
      dispatch(uiActions.setLoaderDialog(true));
      const d: DeleteReservaRequest = {
        reserva_id: data.reserva.id,
        reserva_uuid: data.reserva.uuid,
        establecimiento_uuid: uuid,
      };
      await DeleteReserva(d);
      dispatch(uiActions.setLoaderDialog(false));
      dispatch(dataActions.removeReserva(data.reserva.id));
  ...
  return (
    <>
      {
        <ConfirmationDialog
          open={openDeleteReservaConfimation}
          close={() => setOpenDeleteReservaConfirmation(false)}
          performAction={() => deleteReserva()}
          description="Antes de continuar, por favor, confirme si desea eliminar esta reserva."
        />
      }
      ...
      <DialogLayout
        ...
        close={() => setShowReservaDetail(false)}
      >
        <div className=" overflow-auto">
          ...
          <div className=" border-b-[1px] pb-2">
            <div className="flex space-x-2 items-center">
              ...
              <div className="flex flex-col gap-y-2">
                <Typography data-testid="detail-cancha-name">
                  {detail.instalacion.name}
                </Typography>
                <Chip
                  size="small"
              ...
              <List>
                  ...
                  secondaryAction={
                    detail.reserva.reserva_type != ReservaType.App && (
                      <IconButton
                        ...
                        aria-label="see"
                        size="small"
                        href={getRouteEstablecimiento(
                          uuid,
                          `users/${detail.reserva.user_uuid}?id=${detail.reserva.user_id}&name=${detail.reserva.nombre}`
                        )}
                        target="_blank"
                      >
                ...
                >
                  <ListItemIcon>
                    <CommonImage
                      src={
                        detail.reserva.profile_photo || "/images/profile.png"
                      }
                      h={30}
                      w={30}
                  ...
                  <ListItemText
                    primary={getFullName(
                      detail.reserva.nombre,
                      detail.reserva.apellido
                    )}
                  />
                </ListItem>
              </List>
          ...
          <div className=" mb-4 ">
            <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
              ...
              >
                {moment.utc(detail.reserva.start_date).format("ll")} de{" "}
                {moment.utc(detail.reserva.start_date).format("LT")} a{" "}
                {moment.utc(detail.reserva.end_date).format("LT")}{" "}
              </Typography>
            </div>
            ...
            <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
              ...
              <Typography data-testid="reserva-estado" variant="body2">
                {getEstadoReserva(detail.reserva.estado)}
              </Typography>
            </div>
            ...
            <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
              ...
              <Typography data-testid="reserva-precio" variant="body2">
                {detail.reserva.total_price}
              </Typography>
            </div>
            ...
            <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
              ...
              <Typography data-testid="reserva-monto-pagado" variant="body2">
                {detail.reserva.paid}
              </Typography>
            </div>
            ...
            <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
              <Typography variant="subtitle2" className="label">
                Hora en la que se hizo la reserva
              </Typography>
            ...
            {detail.reserva.evento.id != 0 && (
              <div className="grid sm:grid-cols-2 items-center gap-x-4 border-b-[1px] py-2">
                ...
                <Link
                  href={getRouteEstablecimiento(
                    uuid,
                    `eventos/${detail.reserva.evento.uuid}?name=${detail.reserva.evento.name}&id=${detail.reserva.evento.id}`
                  )}
          ...
          <>
            <DialogActions>
              <div className="flex flex-wrap gap-2">
                ...
                {(detail.reserva.estado == ReservaEstado.Valid ||
                  detail.reserva.estado == ReservaEstado.Pendiente) && (
                  <Button
                ...
                {detail.reserva.estado == ReservaEstado.Cancel &&
                  detail.reserva.paid == 0 && (
                    <Button
                      data-testid="delete-reserva"
                      variant="contained"
                      onClick={() => setOpenDeleteReservaConfirmation(true)}
                    >
                      Eliminar reserva
                    </Button>
                  )}
