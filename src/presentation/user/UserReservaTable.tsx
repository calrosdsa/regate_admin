...
const UserReservaTable = ({
  ...
  return (
    <div className={`relative`}>
      <LinearProgressMui loading={loading} />
      <Paper sx={{ width: "100%" }} elevation={2}>
        <TableContainer>
          <Table>
            <TableHead>
              <TableRow>
                <StyledTableCell className="headerTable w-10"></StyledTableCell>
                ...
                <StyledTableCell className="headerTable"></StyledTableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {reservas.map((item, index) => {
                return (
                  <StyledTableRow key={item.id}>
                    ...
                    <StyledTableCell>{item.instalacion_name}</StyledTableCell>
                    <StyledTableCell>{item.total_price}</StyledTableCell>
                    <StyledTableCell>{item.paid}</StyledTableCell>
                    <StyledTableCell>
                      {getEstadoReserva(item.estado)}
                    </StyledTableCell>

                    <StyledTableCell>
                      {moment(item.start_date).utc().format("ll")} de
                      {moment(item.start_date).utc().format("LT")} a{" "}
                      {moment(item.end_date).utc().format("LT")}
                    </StyledTableCell>
